...
- **見取り図からの自動生成（AI）**：店内の見取り図を撮影・アップロードすると、Google Gemini が画像を解析して壁・商品棚・通路・入口・レジ・階段・エレベーターを自動配置します。棚に書かれた文字（「青果」など）があれば読み取って棚名にし、既存のジャンル辞書で自動判定します。利用にはご自身の Google AI (Gemini) APIキー（[取得はこちら](https://aistudio.google.com/apikey)、クレジットカード登録不要の無料枠で利用可）が必要で、端末内にのみ保存され、画像とともに Google の API に直接送信されます（開発者には送信されません）。生成結果は他の編集操作と同じく「元に戻す」で取り消せます
